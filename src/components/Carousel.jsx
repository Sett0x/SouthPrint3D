    ...
    <div className="relative w-full">
      <div className="relative h-96 overflow-hidden rounded-lg md:h-96">
        {images.map((img, index) => (
          <div key={index} className={`absolute w-full h-full transition-transform duration-700 ease-in-out ${index === currentIndex ? 'opacity-100' : 'opacity-0'}`}>
            <img src={`/products/${img}`} className="absolute inset-0 w-full h-full object-cover" alt={`Slide ${index + 1}`} />
          </div>
        ))}
      </div>
      <div className="flex mt-4 justify-center">
        {images.map((img, index) => (
          ...
          className={`w-24 h-24 mx-1 cursor-pointer rounded-lg ${index === currentIndex ? '  border-4 border-white' : ''}`}
